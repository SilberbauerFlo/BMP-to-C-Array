...
int main(void) {
	FILE *file; //the file for the BMPs name
	FILE *bmp;  //the BMP file
	FILE *cArr; //the output (a header file)

	char BmpName[BMP_NAME_SIZE] = {0}; 	//used to store the name of the BMP
	fpos_t NameLength = 0;				//used to rewrite the BMPs name for further use

	uint32_t BmpWidth = 0;
	uint32_t BmpHeight = 0;
	/*
	 * these variables determine the length of the array used to transfer the data
	 * from the BMP to the header file
	 */

	file = fopen("file.txt","r"); 		//opens the file.txt in reading mode
	fgets(BmpName,BMP_NAME_SIZE,file); 	//reads the BMPs name
	fgetpos(file,&NameLength);			//reads the length of the BMPs name by checking how far the cursor has moved
	fclose(file);

	bmp = fopen(BmpName,"r"); 	//opens the BMP using the name obtained from the file.txt
